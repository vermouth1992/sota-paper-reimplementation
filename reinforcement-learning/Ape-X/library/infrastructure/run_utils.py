...
from .logger import colorize, convert_json
...
class ExperimentManger():
    ...
    def register(cls, func):
        # TODO: make sure fun has no args. Or we can add args in subparser in main
        func_name = func.__name__
        cls.ALL_EXPERIMENTS[func_name] = func

    @classmethod
    def main(cls):
        import argparse
        parser = argparse.ArgumentParser()
        parser.add_argument('exp', type=str, choices=cls.ALL_EXPERIMENTS.keys())

        args = vars(parser.parse_args())
        func = cls.ALL_EXPERIMENTS[args['exp']]
        func()
